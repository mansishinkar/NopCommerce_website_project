...
class TestSearchCustomerByEmail:
    ...
    @pytest.mark.search
    def test_search_customer_using_email(self, set_up):
        self.driver = set_up
        ...
        self.search_customer.set_email("steve_gates@nopCommerce.com")
        self.search_customer.search_button()
        time.sleep(3)

        status = self.search_customer.search_customer_by_email("steve_gates@nopCommerce.com")
        assert True == status
        self.driver.close()
